...
const StatCard: React.FC<{ stat: DashboardStat, isPrivacyMode: boolean }> = ({ stat, isPrivacyMode }) => (
  <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-shadow">
    <div className="flex justify-between items-start">
      <div>
        <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">{stat.label}</p>
        <h3 className="text-xl md:text-2xl font-bold text-invest-900 dark:text-white mt-1">
      ...
      <div className={`p-2 rounded-full ${stat.trend === 'up' ? 'bg-green-50 text-green-600' : stat.trend === 'down' ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-600'}`}>
        {stat.trend === 'up' ? <ArrowUpRight size={18} /> : stat.trend === 'down' ? <ArrowDownRight size={18} /> : <Activity size={18} />}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-2">
        ...
        <div className="text-sm text-slate-500 bg-white dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700 flex items-center gap-2 w-fit">
              ...
              {aiInsights.map(insight => (
                  <div key={insight.id} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/5 hover:bg-white/20 transition-colors cursor-pointer">
                      <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="text-[10px] opacity-60">{insight.timestamp}</span>
                      </div>
                      <p className="text-xs md:text-sm font-medium leading-relaxed">{insight.message}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-4 md:p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
          <div className="flex justify-between items-center mb-6">
             <h3 className="text-lg font-bold text-invest-900 dark:text-white">AUM Forecast</h3>
             <span className="text-[10px] md:text-xs bg-invest-gold/10 text-invest-gold px-2 py-1 rounded font-bold">Predictive Analytics Active</span>
          </div>
